...
def init_db():
    ...
    try:
        ...
        # Migration: add semantic_clusters table
        _migrate_add_semantic_clusters_table(conn)
...
def _migrate_add_semantic_clusters_table(conn):
    """Create semantic_clusters table if it doesn't exist."""
    conn.execute("""
        CREATE TABLE IF NOT EXISTS semantic_clusters (
            ...
            generated_phrases TEXT,
            book_context TEXT,
            created_date TEXT DEFAULT (datetime('now'))
        )
    """)
    conn.execute("""
        CREATE INDEX IF NOT EXISTS idx_semantic_clusters_label
        ON semantic_clusters(cluster_label)
    """)
    conn.execute("""
        CREATE INDEX IF NOT EXISTS idx_semantic_clusters_date
        ON semantic_clusters(created_date)
    """)
...
class SemanticClusterRepository:
    """Data access for semantic_clusters table.

    Stores keyword clusters generated by semantic analysis,
    along with their generated phrases for KDP backend optimization.
    """
    ...
    def add_cluster(self, cluster_label, keywords, relevance_score=None,
                    generated_phrases=None, book_context=None):
        """Add a semantic cluster.

        Args:
            cluster_label: Human-readable label for the cluster.
            keywords: JSON string of keyword list.
            relevance_score: Relevance score (0-1).
            generated_phrases: JSON string of generated phrases.
            book_context: Book context string (title + genre).
        ...
        cursor = self._conn.execute(
            ...
            '(cluster_label, keywords, relevance_score, generated_phrases, book_context) '
            'VALUES (?, ?, ?, ?, ?)',
            (cluster_label, keywords, relevance_score, generated_phrases, book_context),
        )
        self._conn.commit()
    ...
    def get_clusters(self, book_context=None, limit=50):
        """Get semantic clusters, optionally filtered by book context.

        Args:
            book_context: Optional book context string to filter by.
        ...
        if book_context:
            return self._conn.execute(
                'SELECT * FROM semantic_clusters WHERE book_context = ? '
    ...
    def get_latest_clusters(self, book_context, max_age_hours=24):
        """Get recent clusters for a book context (for caching).

        Args:
            book_context: Book context string.
            max_age_hours: Maximum age in hours to consider fresh.

        Returns:
            List of sqlite3.Row objects, or empty list if stale/missing.
        """
        return self._conn.execute(
            'SELECT * FROM semantic_clusters '
            'WHERE book_context = ? '
            ...
            (book_context, f'-{max_age_hours} hours'),
        ).fetchall()
